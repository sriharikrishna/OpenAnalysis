...
OA_ptr<UDDUChainsXAIF> ManagerStandard::performAnalysis(ProcHandle proc, 
    OA_ptr<CFG::Interface> cfg, OA_ptr<UDDUChains::Interface> udChains)
{
  ...
  for (; useIterPtr->isValid(); ++(*useIterPtr)) {
    ...
    // BB for use
    OA_ptr<CFG::Interface::Node> useBB 
        = mStmtToBBMap[mMemRefToStmt[use]];
  ...
  for (; defIterPtr->isValid(); ++(*defIterPtr)) {
    ...
    // BB for def
    OA_ptr<CFG::Interface::Node> defBB 
